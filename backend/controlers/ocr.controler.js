        ...
        for (const [key, quantityText] of Object.entries(keyValuePairs)) {
          ...
          if (product) {
            ...
            const existingCartItem = await Cart.findOne({
              userId: req.userId, // Make sure to use userId instead of user
              "products.productId": product._id, // Match the productId in the products array
            });
        
            if (existingCartItem) {
              // Update the quantity of the existing cart item
              const productIndex = existingCartItem.products.findIndex(
                item => item.productId.toString() === product._id.toString()
              );
              existingCartItem.products[productIndex].quantity += numericQty;
              await existingCartItem.save();
            } else {
              // Create a new cart item if not found
              await Cart.create({
                userId: req.userId, // Use userId here
                products: [
                  {
